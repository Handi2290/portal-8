...
class KHSController extends Controller
{
    ...
    function semester2($semester, $ket)
    {
        $mahasiswa = Auth::guard('mahasiswa')->user();      
        $matkul_pindahan_array = [];
        $nilai_grade_pindahan = [
        ...
        if($ket == "download") {
            $pdf = PDF::loadview('pages.mahasiswa.dashboard.components.timeline_nilai', compact('semester', 'matkul', 'no', 'count', 'ipk'));
            return $pdf->download('download.pdf');
        }
        else {
            return view('pages.mahasiswa.dashboard.components.timeline_nilai', compact('semester', 'matkul', 'no', 'count', 'ipk'));
        }
    }
}