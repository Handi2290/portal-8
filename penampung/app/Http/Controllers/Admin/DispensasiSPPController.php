...
class DispensasiSPPController extends Controller
{
    ...
    public function datatable()
    {
        ...
        foreach ($dispensasi as $list)
        {
            ...
            $list['aksi'] = '
                ...
                <a href="'.route('admin.dispensasi_spp.print', $list->id_dispensasi).'" target="_blank"  class="btn btn-primary btn-sm" title="Print"><i class="fa fa-print"></i></a>
            ';
    ...
    {
        $dispensasi = Dispensasi::leftjoin('m_mahasiswa', 'tbl_dispensasi.nim', 'm_mahasiswa.nim')
                    ->leftjoin('tbl_prodi', 'm_mahasiswa.id_prodi', 'tbl_prodi.id_prodi')
                    ->leftjoin('t_tahun_akademik', 'm_mahasiswa.tahun_akademik', 't_tahun_akademik.tahun_akademik' )
                    ->find($id);

        $petugas = Auth::guard('admin')->user()->nama;

        return view('pages.admin.dispensasi_spp.print.detail_print', compact('dispensasi', 'petugas'));
    }
}
