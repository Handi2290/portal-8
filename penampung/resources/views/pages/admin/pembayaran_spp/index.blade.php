	...
	<section class="content">
		...
		<div class="box box-default">
			<div class="box-header with-border">
				<a href="{{ route('admin.pembayaran_spp.transaksi') }}" class="btn btn-primary">History Pembayaran SPP</a>
					...
					if (data.status == 'success') {
						...
						$.each (data.pembayaran_spp, function (key, val) {
							...
							if (val.status == "<span class='text-danger'>Belum Bayar</span>") {
								...
								var btn_bayar = "<a class='btn btn-primary btn-sm btn-bayar' data-bulan='"+val.key_bulan+"'>Input Tagihan</a>";
								var btn_view_file = '';
							} else {
								var btn_bayar = "<a href='{{ route('admin.pembayaran_spp') }}/"+val.id_pembayaran_spp+"/hapus_pembayaran_spp' class='btn btn-danger btn-sm'><i class='fa fa-trash'></i></a>";
								if(val.file != null) {
									var btn_view_file = `<a href="{{ url('images/spp/') }}/${val.file}" class="btn btn-info btn-sm" target="_blank"><i class='fa fa-image'></i><a>`;
								} else {
									var btn_view_file = '';
								}
							...
							if(val.status_bayar == null) {
								var status_bayar = 'Tagihan Belum Di Input';
							} else if(val.status_bayar == 1) {
								var status_bayar = 'Belum Bayar';
							} else {
								var status_bayar = 'Sudah Bayar';
							}
