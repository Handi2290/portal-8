	...
	<table class="table table-striped table-bordered table-spp">
		<thead>
			<tr>
				...
				<th nowrap class="text-center">Bulan</th>
				<th nowrap class="text-center">Status</th>
				<th nowrap class="text-center">Bayar</th>
				<th nowrap class="text-center">Tanggal bayar</th>
					...
					if (data.status == 'success') {
						...
						$.each (data.pembayaran_spp, function (key, val) {
							if (val.status == "<span class='text-danger'>Belum Bayar</span>") {
								...
								var btn_cek = "<a href='{{ route ('admin.pembayaran_spp') }}/"+val.id_pembayaran_spp+"/show' class='btn btn-primary btn-sm'>Cek Bukti</a>";
							} else {
								(val.status == "<span class='text-success'>Sudah Bayar</span>")
