...
class PengumumanController extends Controller
{
   ...
   public function dosen($pengumuman = null, Request $req)
  {
    ...
    #Pusat Informasi Staff
    public function staff($pengumuman = null, Request $req)
    {
    if(!empty($req->search))
    {
        $pengumuman = Pengumuman::where('is_delete', 'N')
                        ->where('umumkan_ke','!=', 'Staff')
                        ->where('judul_pengumuman', 'LIKE', '%'.$req->search.'%')
                        ->orderBy('waktu_pengumuman','DESC')
                        ->get();
        $search = $req->search;
    ...
    return view('pages.staff.pengumuman.pengumuman', compact('pengumuman'));
    }
