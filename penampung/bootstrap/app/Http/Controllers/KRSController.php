...
class KRSController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('pages.mahasiswa.krs.index', compact('ulang', 'list_tahun_akademik', 'pembukaan_krs', 'thn_skdmk', 'bayaran', 'status'));
    }
    ...
    public function get_krs(Request $request)
    {
        ...
        // Limit KRS untuk mahasiswa yang memiliki tunggakan pembayaran
        $bayaran = DB::select("select 1
                             ...
                             where tts.nim = ".Auth::guard('mahasiswa')->user()->nim."
                               and tta.tahun_akademik ='".$tahun_akademik."'");
        ...
        if (($bayaran >= 1)){
            ...
            return response()->json(['status' => 'error', 'message' => '<b><center><strong class="text-danger">Anda belum melunasi pemabayaran. Silahkan menghubungi bagian keuangan.</strong></center></b>'], 200);
        }
    }
        ...
        else
        {
            ...
            else
            {
                foreach ($request->id_matkul as $key => $matkul)
                {
                    ...
                    $jadwal = Jadwal::where('t_jadwal.tahun_akademik', $krs->tahun_akademik)
                                    ->where('t_jadwal.id_waktu_kuliah',$krs->id_waktu_kuliah)
                                    ->where('t_jadwal.id_prodi',$mahasiswa->id_prodi)
                                    ->where('t_jadwal.id_matkul',$matkul)                            
                                    ->where('t_jadwal.hari',$request->hari[$key])
                                    ->where('t_jadwal.jam_mulai',explode('-', $request->jam[$key])[0])
                                    ->where('t_jadwal.jam_selesai',explode('-', $request->jam[$key])[1])                          
                        ->leftJoin('m_kelas', 't_jadwal.id_kelas', 'm_kelas.id_kelas')
                        ->orderBy('m_kelas.kode_kelas', 'DESC')
                        ->first();
    ...
    public function status (){
        return view('pages.mahasiswa.krs.statusBayaran');
    }
}
